...
void Camera::ShakeCamera(XMFLOAT3 cameraPos)
{
	//シェイクタイマーが０ではないなら
		//シェイクの値をPosに加算しシェイク指せ、シェイクタイマーを経過させる
	if (shakeTimer > 0)
	{
		shake.x = (float)rand() / RAND_MAX * shakeTimer - shakeTimer / 2.0f;
		shake.y = (float)rand() / RAND_MAX * shakeTimer - shakeTimer / 2.0f;
		SetTarget({
			cameraPos.x + shake.x,
			cameraPos.y + shake.y,
			cameraPos.z});
